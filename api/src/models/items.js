    ...
    Mongodb.getDb(function(db){
        ...
        for(let i=0; i<docs.length;i++){
            updates.push(new Promise(function(resolve, reject){
                db.collection(name).updateOne({"project": newDocs[i]["project"],"name": newDocs[i]["name"]},newDocs[i],{upsert: true})
                    .then(function(r){
    ...
    Mongodb.getDb(function(db){
        ...
        for(let i=0; i<docs.length;i++){
            deletes.push(new Promise(function(resolve, reject){
                db.collection(name).deleteOne({"project": docs[i]["project"],"name": docs[i]["name"]})
                    .then(function(r){
