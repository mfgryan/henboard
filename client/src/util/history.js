...
history.lock = () => {history.locked = true};
history.unlock = () => {history.locked = false};
...
history.undo = function(){
    ...
    let undoStack = state.history.find(history => history.project === project).undo;
    let undoChanges = undoStack.length-1 >= 0 ? undoStack[undoStack.length-1].undo : {};
...
history.redo = function(){
    let state = store.getState();
    let project = projects.getCurrentProject(state).project;
    let redoStack = state.history.find(history => history.project === project).redo;
    let redoChanges = redoStack.length-1 >= 0 ? redoStack[redoStack.length-1].redo : [];
...
history.push = function(keys, beforeArray, afterArray){
    if(!history.locked){
        ...
        store.dispatch(pushChange({project: project, keys: keys, redo: redo, undo: undo}));
    }
};
