    ...
    switch (action.type) {
        ...
        case PUSH_CHANGE:
            return state.map(history => {
                return history.project === action.project
        ...
        case REDO:
            return state.map(history => {
                return history.project === action.project
                    ? Object.assign({}, history, {
                          undo: history.undo.concat(
                              history.redo[history.redo.length - 1] || []
                          ),
    ...
    switch (action.type) {
        case UPDATES:
            newKey[action.key] = state[action.key].map(obj => {
                let index = tools().indexOfMatch(
                    models[action.key].primaryKeys,
                    obj,
                    action.data
                );
