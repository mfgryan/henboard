...
describe('lanes reducer', () => {
    ...
    test('toggleAddItem', () => {
        let initialState = [{ project: "henboard", column: "Todo", addItem: false, value: "" }];
        let input = {project: "henboard", column: "Todo"};
        let action = actions.toggleAddItem(input);

        let expectedState = [{ project: "henboard", column: "Todo", addItem: true, value: "" }];
        expect(lanes(initialState, action)).toEqual(expectedState);  
    });
    
    test('addLane', () => {
        let initialState = [{ project: "henboard", column: "Todo", addItem: false, value: "" }];
        let input = { project: "henboard", column: "FooLane", addItem: false, value: "" };
        let action = actions.addLane(input);

        let expectedState = [
            { project: "henboard", column: "Todo", addItem: false, value: "" },
            { project: "henboard", column: "FooLane", addItem: false, value: "" }];
        expect(lanes(initialState, action)).toEqual(expectedState);  
    });
    
    test('changeValue', () => {
        let initialState = [{ project: "henboard", column: "Todo", addItem: false, value: "" }];
        let input = { project: "henboard", column: "Todo", addItem: false, value: "foo" };
        let action = actions.changeValue(input);

        let expectedState = [{ project: "henboard", column: "Todo", addItem: false, value: "foo" }];
        expect(lanes(initialState, action)).toEqual(expectedState);  
    });
