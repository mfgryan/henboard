//action types
import { OPEN_INFO, CLOSE_INFO, ADD_ITEM, REMOVE_ITEM, MOVE_ITEM, CHANGE_ITEM_VALUE, INIT_ITEMS, TOGGLE_INFO_ADD } from "../actions/items";
...
function items(state = [], action){
    switch (action.type){
        ...
        case TOGGLE_INFO_ADD:
            return state.map((item) => {
                return (item.project === action.project && item.name === action.name) ? Object.assign({},item,{addItem: !item.addItem}) : item    
            })
        default:
            return state
    }
